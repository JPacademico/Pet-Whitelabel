...
export function GalleryPage() {
  ...
  const photos = useMemo(
    () => (filter === 'all' ? galleryPhotos : galleryPhotos.filter((p) => p.animalType === filter)),
    [filter],
  );
  ...
  const counts = useMemo(
    () => ({
      ...
      dog: galleryPhotos.filter((p) => p.animalType === 'dog').length,
      cat: galleryPhotos.filter((p) => p.animalType === 'cat').length,
    }),
    [],
  );
